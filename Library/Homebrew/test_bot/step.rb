  ...
  module TestBot
    ...
    class Step
      ...
      def initialize(command, env:, verbose:, named_args: nil, ignore_failures: false, repository: nil)
        ...
        @env = T.let(env, T::Hash[String, String])
        @verbose = T.let(verbose, T::Boolean)
        @ignore_failures = T.let(ignore_failures, T::Boolean)
        ...
        @end_time = T.let(nil, T.nilable(Time))
      end
      ...
      def annotation_location(name)
        ...
        method_sym = T.must(command.second).to_sym
        ...
        [T.must(@repository).glob("**/#{name}*").first, nil]
      end
